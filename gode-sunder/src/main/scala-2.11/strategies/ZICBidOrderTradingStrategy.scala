...
/** Class implementing the Zero Intelligence (ZI) trading strategy from Gode-Sunder (JPE, 1996).
  ...
  */
case class ZICBidOrderTradingStrategy(prng: RandomGenerator,
                                      quantity: Long,
                                      valuations: Map[Tradable, Long])
  extends ZICTradingStrategy[BidOrder] {

  def getPriceDistribution(tradable: Tradable, ticker: Agent[Tick]): Try[RealDistribution] = {
    val valuation = valuations(tradable)
    val marketPrice = ticker.get.price
    if (valuation >= marketPrice) {
      Success(new UniformRealDistribution(prng, marketPrice, valuation))
    } else {
      Failure(???)
    }
  }
